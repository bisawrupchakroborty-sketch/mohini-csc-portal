...
async function storageDelete(path) {
  try { await fbStorage.ref(path).delete(); } catch(e) { console.warn('storageDelete failed:', path, e.message); }
}

async function storageGetUrl(path) {
  try { return await fbStorage.ref(path).getDownloadURL(); } catch(e) { console.warn('storageGetUrl failed:', path, e.message); return null; }
}
